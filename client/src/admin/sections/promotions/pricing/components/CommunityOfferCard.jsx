...
export function CommunityOfferCard({ data, onChange }) {
...
>
      <CardHeader
        avatar={
          ...
          >
            <GroupsOutlinedIcon sx={{ color: "success.main" }} />
          </Box>
        }
        title={<Typography variant="h6" fontWeight={700}>Community Offer</Typography>}
        subheader="Free, donation & low-cost listing tiers"
        action={
          <FormControlLabel
            control={<Switch checked={data.enabled} onChange={handleToggle("enabled")} color="success" size="small" />}
...
>
        <Stack spacing={3}>
          ...
          <Stack spacing={1}>
            <FormControlLabel
              control={
                <Switch
                  checked={data.allowFreeTier}
                  onChange={handleToggle("allowFreeTier")}
                  size="small"
                  color="success"
                  disabled={!data.enabled}
                />
              }
              label={<Typography variant="body2">Allow Free Tier</Typography>}
            />
            <FormControlLabel
              control={
                <Switch
                  checked={data.requireAdminReviewForFree}
                  onChange={handleToggle("requireAdminReviewForFree")}
                  size="small"
                  color="success"
                  disabled={!data.enabled}
                />
              }
              label={<Typography variant="body2">Require Admin Review for Free</Typography>}
            />
            <FormControlLabel
              control={
                <Switch
                  checked={data.allowPaidTiersBelowMinimum}
                  onChange={handleToggle("allowPaidTiersBelowMinimum")}
                  size="small"
                  color="success"
                  disabled={!data.enabled}
                />
              }
              label={<Typography variant="body2">Allow Paid Tiers Below Minimum</Typography>}
            />
          </Stack>
      ...
      <PlanDialog
        ...
        title={editingIndex !== null ? "Edit Community Plan" : "Add Community Plan"}
      />
    </Card>
  );
}

CommunityOfferCard.propTypes = {
